...
-> host-backed availability preflight -> approval when required
-> policy recheck after modified approval -> fresh snapshot and final tool binding
-> final scope/policy/availability checks -> side-effect resource locks
-> generation check and atomic rate admission
-> per-attempt timeout/cancellation around Tool::execute
-> output cap -> hooks, observability, and evidence
```

Policy denial cannot be overridden by approval. Approved arguments are rechecked, and a call is re-resolved and re-authorized after an approval wait. Host-backed `command`, `diagnostics`, and `web_search` availability is checked after initial policy and before HITL, then revalidated during final pre-lock admission. A tool timeout covers one `Tool::execute` invocation attempt, not HITL wait, resource-lock wait, retries, or the complete request; every safely retryable invocation receives its own timeout. Tool-security configuration supplies the baseline, while recovery and call classification may only lower it. Configured values are validated against the accepted range documented in the YAML Reference, and the effective UTC deadline and runtime timer use the same checked duration. Automatic retry is limited to calls classified as safely retryable, and non-idempotent mutation and command calls are not retried as if they were reads. Timeout and cancellation provide no general rollback guarantee.
...
`executed` is `true` only when the tool implementation was invoked. Denied, unavailable, rejected, timed-out, cancelled, failed, and successful requests all retain structured evidence when a final record is produced. `call_id` correlates records from one originating call and its fallback chain rather than identifying each fallback record uniquely. `on_tool_start` and `on_tool_complete` describe the shared-executor request lifecycle, not individual retry invocations. A request can finalize without a start hook or after a start hook without invoking the implementation, so neither hook is execution proof. `ToolExecutionRecord.executed` is authoritative. When recovery selects a fallback tool, the failed original request releases its resource guards and completes its own hook and record lifecycle before the fallback begins as a separate shared-executor request. Runtime-control cancellation remains terminal for that tool request and does not start fallback work. Resolved canonical ancestry is checked at initial admission and after final resolution, and a bounded hop limit prevents direct, alias-mediated, or overlong recursive fallback from invoking tools without bound. The terminal rejected request retains `executed: false` evidence. Eval `tool_called` assertions can inspect execution flags and arguments, while `result_path` parses successful structured output; see [Evaluation](@/docs/evaluation.md).
...
The v1 filesystem contract assumes a host-owned workspace without concurrent untrusted external replacement of validated files, directories, or parent paths. Runtime locks serialize framework-owned conflicting mutations, but they are not OS directory handles or a filesystem sandbox; use process, container, mount, and permission isolation when another process can modify the workspace concurrently. Atomic replacement and patch rollback attempts are tool-specific best efforts, not a runtime-wide rollback guarantee. Timeout, cancellation, external modification, cleanup failure, or process/network side effects can leave externally visible effects.
...
Three host-backed tools have an explicit runtime availability preflight after initial policy and before HITL. Availability is revalidated during final pre-lock admission so a provider removed while approval was in progress cannot be invoked:
