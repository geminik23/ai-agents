...
One `RuntimeAgent` owns one independent root turn at a time across blocking chat, legacy streams, and event streams. Concurrent root-turn calls on the same runtime are serialized so memory, state, hooks, and finalization cannot interleave. Internal redispatch stays inside its current root turn, while a public stream retains turn ownership until terminal completion or drop. Direct nested calls, response hooks, and framework-owned spawned calls such as concurrent orchestration or registry broadcast fail before waiting when their ownership chain already contains the target runtime, including an `A -> B -> A` cycle. Non-cyclic calls to another runtime remain allowed, and separate runtime instances remain independently concurrent.
...
Some provider-native calls carry opaque replay state in addition to the normalized tool ID and arguments. The first-party Google adapter preserves Gemini `thoughtSignature` values by retaining the original model `Content` and returning it with correlated function results on the next request. Replay state never authorizes or executes a call: the normalized call still passes through the shared executor, and approved argument changes affect only the executed request and evidence rather than rewriting the signed model proposal. Losing speculative branches discard their provider state. Current signed exchanges are protected from partial memory eviction, while corrupted or incomplete active history stops locally without automatically repeating an already executed tool.

The split local-file tools block raw `.git` paths, while `git_status` and `git_diff` expose bounded repository inspection. Filesystem mutation tools execute when `dry_run` is omitted. Set `dry_run: true` to request validation or a preview without applying the mutation. Every actual mutation still requires applicable write policy and either approval or an explicit trusted-policy exemption. Results expose `mutation_performed`, while `created`, `overwritten`, `copied`, `moved`, and `deleted` are true only when that effect occurred. Path policy resolves candidates and configured roots through their nearest existing entries before containment checks. Relative roots remain inside the canonical host-owned workspace, explicit absolute roots authorize their resolved locations, dangling symlinks fail closed, and lexical-or-resolved restriction matching prevents a symlink from weakening deny or approval policy while the checked topology remains unchanged. Recursive copy rejects symlinks and destinations nested under the source, and read-before-write version checks apply when configured. The v1 contract assumes a host-owned workspace without concurrent untrusted external path replacement. Non-concurrency-safe path operations use one conservative shared mutation lock; unbound side effects use a separate shared lock. Parent and spawned runtimes share the lock table, but these controls are not an OS filesystem sandbox. Host-backed `diagnostics`, `command`, and `web_search` availability is checked after initial policy and before HITL. After approval, the runtime resolves the final tool once, requires the resolved implementation to match the reviewed tool, reapplies current argument caps, and rechecks scope, policy, emergency control, provider availability, and approval binding before locking. After lock acquisition, a changed policy, runtime-control, or state generation or a failed atomic rate admission prevents invocation. Cached `web_fetch` responses are reused only when their redirect count satisfies the current effective limit, and cache hits repeat current DNS/IP and URL-policy validation. The default transport disables proxies and connects each request only to addresses that passed its DNS/IP checks. Custom low-level transports must expose each redirect response, honor per-response byte limits, and preserve validated-address and host-egress boundaries. The `command` tool is not a shell: it runs exact allowlisted argv commands inside explicit `working_dirs`, starts from an empty environment, filters env through policy, redacts argv evidence, and bounds output. Missing host support for `diagnostics`, `command`, or `web_search` is rejected before approval and implementation invocation, revalidated during final pre-lock admission, and recorded with `executed: false`. `ask_user` is different: its fallback implementation executes and returns a structured unavailable response, using a configured default when present. See [Built-in Tools](@/docs/built-in-tools.md).

A tool timeout applies to each admitted `Tool::execute` attempt. Security, recovery, and call-classification timeout values are validated against the shared accepted range documented in the YAML Reference. Tool security establishes the baseline, while recovery and classification can only lower it. Initial classification is validated before HITL, and classification recomputed from approved arguments is revalidated before locking and invocation. The effective cap produces the UTC deadline and Tokio timer from one checked duration. HITL and resource-lock waits are outside that timer, retries get independent timers, `command.timeout_ms` separately bounds its direct child process, and eval turn timeout covers the whole turn. None of these layers guarantees rollback of effects already visible outside the runtime. Likewise, `on_tool_start` and `on_tool_complete` describe the shared-executor request lifecycle rather than individual retry invocations or implementation invocation; `ToolExecutionRecord.executed` is authoritative. A failed request finalizes before fallback begins, runtime-control tool cancellation does not start fallback work, and canonical ancestry plus a bounded hop limit prevents recursive fallback cycles from running without bound.
...
The key design is rule-based first: state, response text, context, metadata, tools, facts, relationships, persona state, orchestration metadata, and observability summaries should be checked deterministically where possible. Use `judge` or `response_semantic` only when the expected behavior is semantic and cannot be expressed as structured evidence. Eval captures a cursor before each turn and derives `TurnObservabilityEvidence` reports and span IDs only from later events still retained in the manager's rolling metrics window, while direct manager and exporter APIs continue to report their rolling global window.
...
**Error recovery** handles main-response provider failures with configurable retry and exponential backoff, then can use a fallback LLM or static response when the primary retry policy ends in failure. Tool calls use their own retry and failure policy, while context overflow can summarize or truncate prompt history. Compatibility rate-limit and parsing fields do not currently add separate execution paths.
...
**HITL (Human-in-the-Loop)** lets you require human approval before sensitive operations execute. Approval messages support multiple languages, and you can scope approval rules to specific states or conditions. Timeout behavior is configurable - reject, allow, or use a default. An expected rejection can finalize the turn with an authoritative cancellation response. Provider failure, root-future or streaming-task abort, whole-turn timeout, or consumer stream drop can instead end a stream without a final response. Runtime-control tool cancellation is narrower: it stops or denies tool work, while the owning root turn may still recover and produce a final response.
...
A Rust host can also read reports directly. `generate_report()`, raw-event access, and exports use the manager's current rolling global window; eval's captured-cursor turn evidence does not narrow these APIs.
