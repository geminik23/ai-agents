...
A tool-security timeout applies to each admitted `Tool::execute` attempt. HITL and resource-lock waits are outside that timer, retries get independent timers, `command.timeout_ms` separately bounds its direct child process, and eval turn timeout covers the whole turn. None of these layers guarantees rollback of effects already visible outside the runtime. Likewise, `on_tool_start` and `on_tool_complete` describe the shared-executor request lifecycle rather than individual retry invocations or implementation invocation; a request can finalize without a start callback or without invoking the implementation, and `ToolExecutionRecord.executed` is authoritative. When recovery selects a fallback tool, the failed original request releases its resource guards and completes its own hook and record lifecycle before the fallback begins as a separate shared-executor request.
