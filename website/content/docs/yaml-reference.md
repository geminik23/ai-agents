...
| `speculative_skill_routing` | `bool` | `false` | Runs pure skill selection beside a main response draft; skill execution happens only after the skill branch wins. When skills are defined but this flag is off (or the branch cannot be scheduled), skill selection still runs serially after transition routing resolves, so speculation never bypasses a skill |
...
| `streaming_policy` | enum | `preflight_only` | `preflight_only`, `buffer_until_routing_done`, or `disabled`; buffered routing requires `streaming.enabled: true` and positive `streaming.buffer_size`. `preflight_only` and `buffer_until_routing_done` both run pre-response deterministic routing before the stream opens; `disabled` skips every optimized streaming step including preflight, so `timing: pre_response` transitions only fire in blocking turns. Buffered speculative streaming applies to response-independent parallel state-transition routing plus serial skill selection; any non-`none` reasoning mode falls back to the serial streaming path. The buffer limit applies while routing (transition and skill selection) is unresolved |
...
| Let skill selection race a normal response | `speculative_skill_routing` | Define skills and leave no pending skill clarification; set enough speculative call capacity for draft + router | The branch performs pure selection only. Skill disambiguation and skill steps run after the skill branch wins. Whenever the skill branch does not run (flag off, low caps, or scheduling failure) while other branches do, skill selection runs serially after transition routing resolves and still wins over the draft. |
...
| Hide stale stream chunks while a parallel transition is unresolved | `streaming_policy: buffer_until_routing_done` plus `speculative_state_transitions` | Set `streaming.enabled: true`; set positive `streaming.buffer_size`; use a response-independent `timing: parallel` transition; keep `reasoning.mode: none` | Main-stream chunks are hidden while routing is unresolved. If the route or a serially selected skill wins, stale chunks are discarded. The buffer limit applies only while routing is unresolved. Agents with a non-`none` reasoning mode use the serial streaming path instead. |
