...
pub enum ToolRef {
    Simple(String),
    Conditional { id: String, condition: ToolCondition },
}
...
impl StateDefinition {
    ...
    pub fn get_effective_tools<'a>(&'a self, parent: Option<&'a StateDefinition>) -> Vec<&'a ToolRef> {
...
"#;
        let config: StateConfig = serde_yaml::from_str(yaml).unwrap();
        assert!(config.validate().is_ok());
        assert!(config.states.get("problem_solving").unwrap().has_sub_states());
    }
