...
impl StateMachine {
    ...
    fn resolve_to_leaf_state(&self, path: &str) -> Result<String> {
        ...
        loop {
            let def = self
                .config
                .get_state(&current_path)
                .ok_or_else(|| AgentError::InvalidSpec(format!("State not found: {}", current_path)))?;
    ...
    pub fn reset(&self) {
        let initial = Self::resolve_initial_state(&self.config).unwrap_or(self.config.initial.clone());
...
mod tests {
    ...
    fn test_global_transitions() {
        ...
        assert!(transitions.iter().any(|t| t.to == "escalation" && t.priority == 100));
