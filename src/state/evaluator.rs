    ...
    fn compare_value(&self, value: &Value, op: &CompareOp) -> bool {
        match op {
            ...
            CompareOp::Contains(s) => value
                ...
                .map(|v| v.contains(s))
                .or_else(|| {
                    value.as_array().map(|arr| arr.iter().any(|v| v.as_str() == Some(s)))
                })
                .unwrap_or(false),
        }
    }
...
impl GuardOnlyEvaluator {
    ...
    pub fn evaluate_guard(&self, guard: &TransitionGuard, ctx: &TransitionContext) -> bool {
        let evaluator = LLMTransitionEvaluator {
            llm: Arc::new(crate::llm::mock::MockLLMProvider::new("guard_eval")),
        };
        evaluator.evaluate_guard(guard, ctx)
    }

    pub fn evaluate_guards(&self, transitions: &[Transition], ctx: &TransitionContext) -> Option<usize> {
        for (i, transition) in transitions.iter().enumerate() {
            if let Some(ref guard) = transition.guard {
...
mod tests {
    ...
    async fn test_compare_contains() {
        ...
        context_map.insert(
            "tags".to_string(),
            serde_json::json!(["urgent", "support"]),
        );
