...
use ai_agents_state::{CompareOp, ContextMatcher, StateMatcher, TimeMatcher, ToolCondition};
...
impl<G: LLMGetter> ConditionEvaluator<G> {
    ...
    pub async fn evaluate(
        ...
        match condition {
            ...
            ToolCondition::Not(inner) => Ok(!Box::pin(self.evaluate(inner, ctx)).await?),
        }
    }
    ...
    fn evaluate_state(&self, matcher: &StateMatcher, ctx: &EvaluationContext) -> bool {
        ...
        if let Some(ref turn_op) = matcher.turn_count {
            ...
            if !self.compare_value(
                &Value::Number(serde_json::Number::from_f64(turn_count).unwrap_or(0.into())),
                turn_op,
            ) {
                return false;
            }
...
{{"result": false, "confidence": 0.9, "reason": "brief explanation"}}"#,
        ...
        let parsed: SemanticEvalResult =
            serde_json::from_str(&response.content).unwrap_or(SemanticEvalResult {
                result: false,
                confidence: 0.0,
...
mod tests {
    ...
    async fn test_all_condition() {
        ...
        let condition =
            ToolCondition::All(vec![ToolCondition::Context(m1), ToolCondition::Context(m2)]);
    ...
    async fn test_any_condition() {
        ...
        m2.insert(
            "tier".to_string(),
            ContextMatcher::Exact(Value::String("basic".into())),
        );

        let condition =
            ToolCondition::Any(vec![ToolCondition::Context(m1), ToolCondition::Context(m2)]);
