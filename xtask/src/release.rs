...
mod tests {
    ...
    fn wrong_package_version_is_rejected() {
        ...
        metadata.packages[0].version = "1.0.2".to_string();
        let rejections = identity_rejections(&metadata, "", "same-lock", &RELEASE_ORDER);
        assert!(
            rejections
                .iter()
                .any(|reason| reason.contains("has version '1.0.2'"))
        );
    }
